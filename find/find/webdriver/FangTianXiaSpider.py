...
class FangTianXiaSpider(scrapy.Spider):
    ...
    def parse(self, response,city,quyu,quyuchirld):
        ...
        sale_state = response.css('.fangyuan pr span::text').extract()#待售
        ...
        for namee in names:
            ...
            if pquyuchirld!="":
                ...
                allxiaoqu[pcity][pquyu][pquyuchirld][name]["sale_state"] = sale_state[iii].strip()
            else:
              ...
              allxiaoqu[pcity][pquyu][name]["sale_state"] = sale_state[iii].strip()
        ...
        if nexthref==[]:
            fo = open("fangtianxia.txt", "w+")  # 存入文件中。。。
